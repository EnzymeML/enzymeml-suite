...
export default function EquationInput(
    ...
    // Effects
    useEffect(() => {
        convertSpeciesIDsToUnderscore(equation).then(
            (converted) => setLatexEquation(converted)
        ).catch((error) => {
        ...
        <div className={"flex flex-row place-items-center justify-start h-auto"}>
            ...
            </math-field>
            {
                <math-field className={"w-full"}
                            ...
                            style={leftHandStyle}
                >
                    {latexEquation.length > 0 ? latexEquation : "Enter \\: equation \\: here"}
                </math-field>
            }
        </div>);
}